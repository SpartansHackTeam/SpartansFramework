#!/bin/bash
# Install script for Spartans Framework
...
LOOT_DIR=/usr/share/spartansframework/loot
PLUGINS_DIR=/usr/share/spartansframework/plugins
GO_DIR=~/go/bin

echo -e "$OKGREEN + -- --=[ This script will install spartansframework under $INSTALL_DIR. Are you sure you want to continue? (Hit Ctrl+C to exit)$RESET"
if [[ "$1" != "force" ]]; then
	read answer
fi
...
chown root $INSTALL_DIR/spartansframework 2> /dev/null
chmod 4777 $INSTALL_DIR/spartansframework 2> /dev/null
...
git clone https://github.com/SpartansHackTeam/BruteX.git
git clone https://github.com/SpartansHackTeam/Findsploit.git
...
ln -s /usr/share/spartansframework/plugins/theHarvester-3.0.6/theHarvester.py /usr/bin/theharvester
...
mv ~/.spartansframework.conf ~/.spartansframework.conf.old 2> /dev/null
cp $INSTALL_DIR/spartansframework.conf ~/.spartansframework.conf 2> /dev/null
...
chmod +x $INSTALL_DIR/spartansframework
chmod +x $PLUGINS_DIR/Goohak/goohak
rm -f /usr/bin/spartansframework
rm -f /usr/bin/goohak
rm -f /usr/bin/dirsearch
ln -s $INSTALL_DIR/spartansframework /usr/bin/spartansframework
...
cp -f $INSTALL_DIR/spartansframework.desktop /usr/share/applications/ 2> /dev/null
cp -f $INSTALL_DIR/spartansframework.png /usr/share/pixmaps/ 2> /dev/null
echo -e "$OKORANGE + -- --=[ Done! $RESET"
echo -e "$OKORANGE + -- --=[ To run, type 'spartansframework'! $RESET"

...
if [[ "$MODE" = "flyover" ]]; then
  ...
  if [[ "$REPORT" = "1" ]]; then
    ...
    if [[ "$SLACK_NOTIFICATIONS" == "1" ]]; then
      /bin/bash "$INSTALL_DIR/bin/slack.sh" "[spartansht.online] •?((¯°·._.• Started Spartansframework scan: $FILE [$MODE] (`date +"%Y-%m-%d %H:%M"`) •._.·°¯))؟•"
    fi
    args=""
    ...
    if [[ "$SLACK_NOTIFICATIONS" == "1" ]]; then
      /bin/bash "$INSTALL_DIR/bin/slack.sh" "[spartansht.online] •?((¯°·._.• Finished Spartansframework scan: $FILE [$MODE] (`date +"%Y-%m-%d %H:%M"`) •._.·°¯))؟•"
    fi
  fi
  exit
fi

...
if [[ "$REPORT" = "1" ]]; then
  ...
  echo "spartansframework -t $TARGET -m $MODE --noreport $args" >> $LOOT_DIR/scans/$TARGET-normal.txt 2> /dev/null
  echo "spartansframework -t $TARGET -m $MODE --noreport $args" >> $LOOT_DIR/scans/running-$TARGET-normal.txt 2> /dev/null
  if [[ "$SLACK_NOTIFICATIONS" == "1" ]]; then
    /bin/bash "$INSTALL_DIR/bin/slack.sh" "[spartansht.online] •?((¯°·._.• Started Spartansframework scan: $TARGET [normal] (`date +"%Y-%m-%d %H:%M"`) •._.·°¯))؟•"
  fi
  spartansframework $args | tee $LOOT_DIR/output/spartansframework-$TARGET-`date +"%Y%m%d%H%M"`.txt 2>&1
  exit
fi
...
if [[ "$SLACK_NOTIFICATIONS" == "1" ]]; then
  /bin/bash "$INSTALL_DIR/bin/slack.sh" "[spartansht.online] •?((¯°·._.• Finished Spartansframework scan: $TARGET [normal] (`date +"%Y-%m-%d %H:%M"`) •._.·°¯))؟•"
fi

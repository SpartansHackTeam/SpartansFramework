      if [[ "$SLACK_NOTIFICATIONS" == "1" ]]; then
            /bin/bash "$INSTALL_DIR/bin/slack.sh" "[spartansht.online] •?((¯°·._.• Started spartansframework webpwn scan: $TARGET [$MODE] (`date +"%Y-%m-%d %H:%M"`) •._.·°¯))؟•"
      fi
      ...
      if [[ "$SLACK_NOTIFICATIONS" == "1" ]]; then
            /bin/bash "$INSTALL_DIR/bin/slack.sh" "[spartansht.online] •?((¯°·._.• Finished spartansframework webpwn scan: $TARGET [$MODE] (`date +"%Y-%m-%d %H:%M"`) •._.·°¯))؟•"
      fi

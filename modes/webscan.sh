if [[ "$MODE" = "webscan" ]]; then
    ...
    if [[ "$ARACHNI_SCAN" == "1" ]]; then
		...
		arachni --report-save-path=$LOOT_DIR/web/http-$TARGET/ --output-only-positives http://$TARGET | tee $LOOT_DIR/output/spartansframework-$TARGET-webscan-http-`date +"%Y%m%d%H%M"`.txt 2>&1
		arachni --report-save-path=$LOOT_DIR/web/https-$TARGET/ --output-only-positives https://$TARGET | tee $LOOT_DIR/output/spartansframework-$TARGET-webscan-https-`date +"%Y%m%d%H%M"`.txt 2>&1

		if [[ "$SLACK_NOTIFICATIONS_ARACHNI_SCAN" == "1" ]]; then
			bin/bash "$INSTALL_DIR/bin/slack.sh" postfile "$LOOT_DIR/output/spartansframework-$TARGET-webscan-http-`date +"%Y%m%d%H%M"`.txt"
			bin/bash "$INSTALL_DIR/bin/slack.sh" postfile "$LOOT_DIR/output/spartansframework-$TARGET-webscan-https-`date +"%Y%m%d%H%M"`.txt"
		fi
